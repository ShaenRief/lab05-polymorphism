...
final class Complex : CustomStringConvertible, StringConvertibleNum {
    ...
    /**
     Copy Method
     - parameter x: Complex number to copy
     */
    func copy() -> Complex {
...
func +(c: Complex, x: Float) -> Complex {
    let cmp: Complex = Complex(real: x, imag: 0)
    return Complex.add(c1: c, to: cmp)
}
...
func -(c: Complex, x: Float) -> Complex {
    let cmp: Complex = Complex(real: x, imag: 0)
    return Complex.subtract(c1: c, from: cmp)
}
...
func *(c: Complex, x: Float) -> Complex {
    let cmp: Complex = Complex(real: x, imag: 0)
    return Complex.multiply(c1: c, by: cmp)
}
...
func /(c: Complex, x: Float) -> Complex {
    let cmp: Complex = Complex(real: x, imag: 0)
    return Complex.divide(c1: c, by: cmp)
}
